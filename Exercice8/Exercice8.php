...
include 'header.php'; ?>
<form action="Exercice8.php" method="get" class="formy">
<fieldset></fieldset>
...
<?php if ((!empty($_GET['nom']))&&(!empty($_GET['prenom']))&&(!empty($_GET["civilite"]&&(!empty($_GET['file']))))){
    $check = pathinfo($_GET['file'], PATHINFO_EXTENSION);
    if ($check == 'pdf'){ ?>
        <p>Ce fichier est bien un pdf</p>        
    <?php } else { ?>
        <p>Ce n'est pas un fichier pdf</p>
    <?php }
    echo $_GET['civilite'].$_GET['prenom'].$_GET['nom'].$_GET['file'].$check;$finfo.$fmime;
     ?>
